import type { ProjectPdfSnapshot } from "./pdfSnapshotTypes";
 ...
 * Cover + TOC + one section per shoot day with page-break-before.
 */
export function renderProjectHtml(snapshot: ProjectPdfSnapshot, baseUrl: string): string {
  ...
  const totalScenes = sortedDays.reduce(
    (sum, d) => sum + (dayData[d.id]?.scenes?.length ?? 0),
    0
  );
  ...
  const tocLines = sortedDays
    .map(
      (day, i) =>
        `    <div class="toc-line">${i + 1}. ${escapeHtml(day.title || "יום צילום")}${day.date ? ` — ${ltrSpan(day.date)}` : ""}</div>`
    )
    .join("\n");
  ...
  const daySections = sortedDays.map((day, dayIndex) => {
    ...
    const dayTitle = `יום צילום ${dayIndex + 1} — ${day.title || "יום צילום"}`;
    const notesBlock = day.generalNotes?.trim()
      ? `<div class="notes-block"><strong>הערות כלליות:</strong><br/>${escapeHtml(day.generalNotes.trim())}</div>`
      : "";
    const daySectionClass = dayIndex === 0 ? "day-section day-section-first" : "day-section";

    return `
  <div class="${daySectionClass}">
    <div class="day-title">${escapeHtml(dayTitle)}</div>
    ${day.date ? `<div class="day-meta">${ltrSpan(day.date)}</div>` : ""}
    ${notesBlock}
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .cover-title { font-size: 20pt; font-weight: 700; margin-bottom: 8px; }
    .cover-sub { font-size: 14pt; font-weight: 700; margin-top: 24px; margin-bottom: 4px; }
    .cover-meta { font-size: 11pt; color: #444; margin: 4px 0; }
    .toc-title { font-size: 14pt; font-weight: 700; margin: 16px 0 8px; }
    .toc-line { font-size: 11pt; margin: 4px 0; }
    .day-section { page-break-before: always; padding-top: 8px; }
    .day-section-first { page-break-before: avoid; }
    .day-title { font-size: 14pt; font-weight: 700; margin-bottom: 4px; }
    .day-meta { font-size: 11pt; color: #444; margin-bottom: 8px; }
...
<body>
  <div class="cover-title">דוח הפקה – פרויקט</div>
  <div class="cover-sub">${escapeHtml(project.name)}</div>
  ${project.clientName ? `<div class="cover-meta">${escapeHtml(project.clientName)}</div>` : ""}
  <div class="cover-meta">נוצר: ${ltrSpan(generatedAt)}</div>
  <div class="cover-meta">מספר ימי צילום: ${ltrSpan(String(sortedDays.length))}</div>
  <div class="cover-meta">סה״כ סצינות: ${ltrSpan(String(totalScenes))}</div>
